...
<form method="POST" action="./back_office/gestion/compte/updateCompte.php">
	    <input type="hidden" name="ID_COMPTE" value="<?php echo $entreprise->getId(); ?>">
        <input type="hidden" value="<?php $datecrea = DateTime::createFromFormat("Y-m-d", $entreprise->getDate()); echo $datecrea->format('d/m/Y'); ?>" name="DATE_CREATION"/>
		<a> Login    : <input type="text" value="<?php echo $entreprise->getLogin(); ?>" id="login" name="LOGIN" /> Minimum 3 Caractères</a><br />
		<a> Mail     : <input type="text" value="<?php echo $entreprise->getMail(); ?>" id="mail" name="EMAIL"/> </a><br />
		<a> Password : <input type="text" value="<?php echo $entreprise->getMdp(); ?>" id="password" name="MOT_DE_PASSE"/> Minimum 8 Caractères (1maj, 1chiffre, 1caractere special)</a><br />
